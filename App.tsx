...
const App: React.FC = () => {
    ...
    const fetchStats = async () => {
      try {
        ...
        if (user.role === "admin") {
          ...
          setPendingSubmissionsCount(
            subs.filter(
              (s) => s.status === "pending" || s.status === "event_pending",
            ).length,
          );
        }
      } catch (e) {}
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0E0E17] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-[#A1A1B3] font-black uppercase tracking-[0.4em] text-[10px] mt-10">
          Synchronizing GRAPE Ecosystem...
        </p>
      </div>
    );
  }
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "Students":
        return <StudentsView searchQuery={searchQuery} user={user} />;
      case "Challenges":
        return <ChallengesView searchQuery={searchQuery} user={user} />;
      case "Messages":
        return <MessagesView user={user} />;
      case "Submissions":
        return isAdmin ? (
          <SubmissionsReviewView searchQuery={searchQuery} user={user} />
        ) : (
      ...
      case "Sticker Store":
        return isAdmin ? (
          <StickerStoreView searchQuery={searchQuery} />
        ) : (
          <DashboardOverview user={user} />
        );
      case "Settings":
        return <SettingsView user={user} onUpdate={(u) => setUser(u)} />;
      default:
        return <DashboardOverview user={user} />;
    }
  };
          ...
          <div className="bg-red-500 text-white py-6 px-8 flex flex-col md:flex-row items-center justify-between animate-in slide-in-from-top-full duration-500 relative z-20 shadow-2xl gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.2em] leading-tight">
                  Column 'started_at' Missing
                </p>
                <p className="text-[9px] opacity-80 uppercase font-bold mt-1">
                  DO NOT DELETE your database. Just run the Fix SQL below.
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => {
                  const sql =
                    "ALTER TABLE public.submissions ADD COLUMN IF NOT EXISTS started_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();";
                  navigator.clipboard.writeText(sql);
                  alert(
                    "One-Line Fix Copied! Paste it into Supabase SQL Editor and click RUN.",
                  );
                }}
